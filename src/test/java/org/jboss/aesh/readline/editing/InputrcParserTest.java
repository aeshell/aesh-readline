...
public class InputrcParserTest {
    ...
    public void testParseInputrc2() throws IOException {
        if(Config.isOSPOSIXCompatible()) {  //TODO: must fix this for windows
            ...
            assertEquals("backward-char", editMode.parse( actionQueue.next()).name());
            actionQueue.add(new int[]{27, 91, 66});
            assertEquals("next-history", editMode.parse( actionQueue.next()).name());
            actionQueue.add(1);
            assertEquals("beginning-of-line", editMode.parse( actionQueue.next()).name());
            ...
            actionQueue.add(new int[]{27, 91, 68});
            assertEquals("forward-char", editMode.parse( actionQueue.next()).name());
            actionQueue.add(new int[]{27, 91, 66});
            assertEquals("previous-history", editMode.parse(actionQueue.next()).name());
            actionQueue.add(new int[]{27,10});
            assertEquals("backward-char", editMode.parse(actionQueue.next()).name());
            actionQueue.add(1);
            assertEquals("forward-word", editMode.parse(actionQueue.next()).name());
        }
    }
