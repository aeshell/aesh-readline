...
public class ANSI {
    ...
    // =========================================================================
    // Batched terminal probe: DECRQM + DA1 fence
    // =========================================================================

    /**
     * Builds a batched query string that probes multiple DEC private modes
     * via DECRQM, terminated by a DA1 request as a fence.
     * <p>
     * The DA1 response ({@code CSI ? <params> c}) acts as a sentinel:
     * when it arrives, all DECRPM responses the terminal will send have
     * already been received.
     *
     * @return the batched query string
     */
    public static String buildBatchedProbeQuery() {
        return MODE_2026_QUERY + MODE_2027_QUERY + DA1_QUERY;
    }

    /**
     * Parse a DECRPM (DEC Private Mode Report) response for any mode.
     * <p>
     * Expected format: {@code ESC [ ? <mode> ; <Ps> $ y}
     * <p>
     * Returns the mode number and Ps value, or null if not a DECRPM.
     *
     * @param response the response string to parse
     * @param startIndex the starting index in the string
     * @return int array {mode, Ps, endIndex} or null if not a DECRPM
     */
    public static int[] parseDECRPM(String response, int startIndex) {
        // Look for ESC [ ? <digits> ; <digit> $ y
        int i = startIndex;
        int len = response.length();

        // Skip to ESC
        while (i < len && response.charAt(i) != '\u001B')
            i++;
        if (i + 6 >= len)
        ...
        // Parse mode number
        int modeStart = i;
        while (i < len && Character.isDigit(response.charAt(i)))
            i++;
        if (i == modeStart || i >= len || response.charAt(i) != ';')
            return null;

        int mode = Integer.parseInt(response.substring(modeStart, i));
        i++; // skip ;

        // Parse Ps value
        if (i >= len || !Character.isDigit(response.charAt(i)))
            return null;
        int ps = response.charAt(i) - '0';
        i++;

        // Expect $ y
        if (i + 1 >= len || response.charAt(i) != '$' || response.charAt(i + 1) != 'y')
            return null;
        i += 2;

        return new int[] { mode, ps, i };
    }

    /**
     * Check if the response string contains a DA1 response starting at or
     * after the given index.
     * <p>
     * DA1 response format: {@code ESC [ ? <params> c}
     *
     * @param response the response string
     * @param startIndex the starting index
     * @return the end index after the DA1 response, or -1 if not found
     */
    public static int findDA1Response(String response, int startIndex) {
        int i = startIndex;
        int len = response.length();
        ...
        while (i < len) {
            ...
            if (response.charAt(i + 1) == '[') {
                // CSI sequence — check if it ends with 'c'
                int seqStart = i;
                i += 2; // skip ESC [
                // Skip params and intermediates
                while (i < len && response.charAt(i) >= 0x20 && response.charAt(i) <= 0x3F)
                    i++;
                // Final byte
                if (i < len && response.charAt(i) == 'c') {
                    return i + 1; // end index after DA1
                }
                i = seqStart + 1; // not DA1, continue searching
            } else {
                i++;
            }
        }
