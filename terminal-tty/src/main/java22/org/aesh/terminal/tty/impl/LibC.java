 ...
 * {@code ioctl}, {@code open}, {@code close}, {@code read}, {@code poll},
 * and {@code sigaction}.
 * <p>
...
final class LibC {
    ...
    static {
        ...
        POLL = LINKER.downcallHandle(
                ...
                FunctionDescriptor.of(
                        ...
                        ValueLayout.JAVA_LONG,      // nfds (unsigned long on 64-bit)
        ...
        SIGACTION = LINKER.downcallHandle(
                STDLIB.find("sigaction").orElseThrow(),
                FunctionDescriptor.of(
                        ValueLayout.JAVA_INT,       // return
                        ValueLayout.JAVA_INT,       // signum
                        ValueLayout.ADDRESS,        // act
                        ValueLayout.ADDRESS),       // oldact
                ERRNO_STATE);
    }
     ...
     */
    static int poll(MemorySegment pollfd, long nfds, int timeout) {
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment capturedState = arena.allocate(Linker.Option.captureStateLayout());
     ...
     *
     * @param signum the signal number (e.g., SIGWINCH)
     * @param act the new signal action (or NULL to just query)
     * @param oldact receives the previous signal action (or NULL)
     * @return 0 on success, -1 on error
     */
    static int sigaction(int signum, MemorySegment act, MemorySegment oldact) {
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment capturedState = arena.allocate(Linker.Option.captureStateLayout());
            return (int) SIGACTION.invokeExact(capturedState, signum, act, oldact);
        } catch (Throwable t) {
    ...
    /**
     * Creates an upcall stub for a signal handler function.
     * The handler signature is: void handler(int signum)
     *
     * @param arena the arena to allocate the stub in (must outlive the registration)
     * @param handler the Runnable to invoke when the signal fires
     * @return a MemorySegment that can be used as sa_handler in struct sigaction
     */
    static MemorySegment createSignalHandlerStub(Arena arena, Runnable handler) {
        try {
            // Create a method handle for the target: void handle(int signum)
            var mh = java.lang.invoke.MethodHandles.lookup()
                    .findVirtual(Runnable.class, "run",
                            java.lang.invoke.MethodType.methodType(void.class))
                    .bindTo(handler);
            // Drop the int signum parameter since Runnable.run() takes no args
            mh = java.lang.invoke.MethodHandles.dropArguments(mh, 0, int.class);
            ...
            throw new RuntimeException("Failed to create signal handler stub", t);
        }
    }
