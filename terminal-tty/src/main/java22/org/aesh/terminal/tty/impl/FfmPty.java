...
public class FfmPty implements Pty {
    ...
    private void attributesToTermios(Attributes attr, MemorySegment termios) {
        ...
        // Control flags: start from current value, mask out known bits, set requested
        long cflag = getTermiosFlag(termios, PosixConstants.TERMIOS_CFLAG_OFFSET);
        // Clear all bits we manage
    ...
    /**
     * OutputStream that writes to the tty file descriptor.
     * Uses System.out as the output stream since writing to a tty fd
     * opened with O_RDWR can have issues with output routing.
     */
    private class FfmOutputStream extends OutputStream {
        ...
        FfmOutputStream() {
            // Use System.out for output — this is what ExecPty does when
            // the tty file can't be opened. Writing directly to the fd
            // works but System.out is more reliable for terminal output
            // since it handles buffering and encoding correctly.
            if (ownsFd) {
                try {
