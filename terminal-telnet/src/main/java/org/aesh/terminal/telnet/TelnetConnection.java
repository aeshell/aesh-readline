...
public abstract class TelnetConnection {
     ...
     * otherwise a <code>DON'T</code> will be sent to the client.
     * <p>
     *
     ...
     * otherwise a <code>WON'T</code> will be sent to the client.
     * <p>
     *
     * This method can be subclassed to handle an option.
     *
            ...
            void handle(TelnetConnection session, byte b) {
                if (session.paramsOptionCode == null) {
                    ...
                    if (session.paramsIac) {
                        ...
                        if (b == BYTE_SE) {
                            try {
                                session.onOptionParameters(session.paramsOptionCode,
                                        Arrays.copyOf(session.paramsBuffer, session.paramsLength));
                            } finally {
