...
/**
 * Kill the text in the current region. The killed text is saved
 * on the kill ring.
 */
public class KillRegion implements Action {
    ...
    public void accept(InputProcessor inputProcessor) {
        ...
        int start = Math.min(cursor, mark);
        int end = Math.max(cursor, mark);
        ...
        inputProcessor.buffer().delete(end - start);
        buffer.setMark(-1);
    }
}
